...
public class TXmlDocument {
	...
	private static final String[] RESPONSIBLE = { "Døívìjší majitel",
			"Nakladatel", "Knihkupec", "Tiskaø ilustrace" };
	private static final String[] TERM_TYPE = { "person_name_religion",
			"person_name", "geographical_name", "topic", "genre", "keyword", "date" };
	private static final String[] TERM_TYPE_TEXT = { "Osoba (náboženská)",
			"Osoba", "Místo", "Téma", "Žánr", "Klíèové slovo", "Datum" };
	private static final String[] LANGS_VAL = { "CZE", "GER", "LAT", "SLO", "POL" };
	private static final String[] LANGS_VAL_LONG = { "èeština", "nìmèina",
			"latina", "slovenština", "polština" };
				...
				if (warning) {
					...
					if (firstchild.getNodeValue().isEmpty()) {
						messageBox.setMessage("Text '" + text
								+ "' na neoèekávaném místì. Pøesunout?");
					} else {
						messageBox
								.setMessage("Text '"
										+ text
										+ "' na neoèekávaném místì. Spojit s správnì umístìným textem '"
		...
		public TAdditional(Node parent, Composite sibling) {
			...
			if (!isComposite) { // Knihopis and other exemplars
				...
				new Label(compositeIdno, SWT.NONE)
						.setText("forma dle èísla ve formátu Knihopisu Digital (K13172)");
	...
	class THistory {
		...
		public THistory(Node parent, Composite sibling) {
			...
			// }
			/*
			 * <origin><p> <!-- Zemì pùvodu --> <placeName
			 * type="country">Èesko</placeName>
			 * 
			 * <!-- DATACE TISKU (rukopisu) --> <origDate notBefore="1780"
			 * notAfter="1820">pøelom 18. a 19. století(</origDate> </p></origin>
			 */
	...
	class TmsSubItem {
		...
		public TmsSubItem(Node parent, Composite p, TContent c) {
			...
			/*
			 * new Label (composite, SWT.NONE).setText("Pøítisk"); new Label
	...
	class TMelody {
		...
		private void Construct(Group gpParent, Composite sibling, Node parent) {
			...
			comboMelodyNote.setItems(new String[] { "Bez nápìvu", "Zpívá se jako:",
					"Jako:" });
		...
		public TmsParts(Node parent, Composite sibling) {
			...
			while (msPart != null) {
				if (msPart.getNodeName().equals("msPart")) {
					 ...
					 * btOpen.setText("Otevøít ..."); btOpen.addSelectionListener(new
					 * OpenMsPart(msPart));
					 * 
	...
	class TContent {
		...
		public TContent(Node parent, Composite sibling) {
			...
			multiLineComposite.setLayout(new GridLayout(2, false));
			// rubric
			/* TMultiLineNode rubric = */new TMultiLineNode("Pøepis titulní strany",
		...
		class TTerm {
			...
			public TTerm(final Node t, Boolean genre) {
				...
				if (genre) {
					...
					combo.setItems(new String[] { "Kramáøská píseò", "Kramáøský tisk",
							"Modlitba" });
	...
	void CreateTitle(Node parent, Composite compParent) {
		...
		new Label(compParent, SWT.NONE).setText(isComposite ? "Název konvolutu"
				: "Titul v pøepisu");
	...
	private void AddLanguage(Node Parent, Composite composite) {
		...
		comboLang.setItems(new String[] { "èesky", "nìmecky", "latinsky",
				"slovensky", "polsky" });
	...
	class TTeiHeaderFrame {
		...
		public TTeiHeaderFrame(Node p) {
			 ...
			 * (composite, SWT.NONE).setText (isComposite ? "Název konvolutu" :
			 * "Titul v pøepisu"); Text textHeadingTitle = new Text (composite,
			 ...
			 * labelAddAuthor.setText ("Další autor"); Button buttonAddAuthor = new
			 * Button(composite, SWT.PUSH); buttonAddAuthor.setText("pøidat ...");
	...
	public void closeDocument() {
		...
		if (WasEdited) {
			...
			messageBox.setMessage("Záznam '" + documentName
					+ "' není uložen. Uložit?");
